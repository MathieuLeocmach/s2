...
%}
}\hfill\textit{J. Chem. Phys. 138, 12A536 (2013)}\\
...
\begin{minipage}[t][3\TPVertModule]{6\TPHorizModule}
	...
	\end{tikzpicture}
	Monte Carlo simulations, N=4000 hard spheres of polydispersity $\Delta$
\end{minipage}
\hfill
...
\begin{tikzpicture}
	\begin{axis}[%
		...
		ylabel={Pressure $\beta p\sigma^3$},
...
\colorbox{lightgray}{\begin{minipage}[t][3\TPVertModule]{3\TPHorizModule}
	...
	\Subhead{Increasing correlation length}
	\begin{itemize}
		\item Many-body order parameter
		\item Specific to crystal-like order
	\end{itemize}
	\Subhead{Two-body is not enough}
	\begin{itemize}
		\item No growing length scale
		\item Not correlated to many-body
	\end{itemize}
	\Subhead{Effect of polydispersity}
	\begin{itemize}
		\item Stabilises icosahedra against crystal
	\end{itemize}
\end{minipage}}
...
%\begin{minipage}[t][12.5\TPVertModule]{15\TPHorizModule}
	...
	\end{groupplot}
	\node[below left=0 of dis c6r1.outer south east, inner xsep=0, text width=\textwidth] (tenper){\Head{Spatial correlation of the 10\% ``most ordered'' particles}};
	\begin{scope}[every node/.style={inner sep=1pt, draw=gray!20, thick, anchor=north west}]
		\node at (dis c1r1.south west|-tenper.south) {\includegraphics[width=0.14\columnwidth]{s2_slice_3D}};
		\node at (dis c2r1.south west|-tenper.south) {\includegraphics[width=0.14\columnwidth]{f2_slice_3D}};
		\node at (dis c3r1.south west|-tenper.south) {\includegraphics[width=0.14\columnwidth]{q6_slice_3D}};
		\node at (dis c4r1.south west|-tenper.south) {\includegraphics[width=0.14\columnwidth]{w6_slice_3D}};
		\node at (dis c5r1.south west|-tenper.south) {\includegraphics[width=0.14\columnwidth]{Q6_slice_3D}};
		\node at (dis c6r1.south west|-tenper.south) {\includegraphics[width=0.14\columnwidth]{C_slice_3D}};
	\end{scope}
	\end{tikzpicture}
%\end{minipage}

\begin{minipage}[b][5\TPVertModule]{7\TPHorizModule}
\begin{tikzpicture}
...
% 	\end{axis}
	...
	\end{tikzpicture}
\end{minipage}
\hspace{\TPHorizModule}
\begin{minipage}[b][5\TPVertModule]{7\TPHorizModule}
\Head{No correlation between two-body and many-body}
\begin{tikzpicture}[
		...
		\begin{groupplot}[
			...
			width=3\TPHorizModule-2em,%
			height=3\TPHorizModule-2em,
			ylabel=$f_2$,%
		...
		\nextgroupplot[%
			xlabel=$Q_6$,%
			xmin=0,xmax=0.6, xtick={0,0.2,...,0.6},%
			]
			\addplot graphics
		...
		\node[single arrow, draw, rotate=90] at ($(X c1r1)!0.5!(X c2r1)$) {low free volume};
		\node[above=of X c1r1, single arrow, draw, anchor=base] {crystal-like order};
		\node[above=of X c2r1, single arrow, draw, shape border rotate=180, anchor=base] {icosahedral order};
		
%		%%color scales by hand
		\matrix (l1) [matrix anchor=east, matrix of nodes, minimum width=1em, minimum height=2em, inner sep=0, draw=black, xshift=-1em] at (X c1r1.left of west)
		{
			|[fill=black]| \\ 
			|[fill=black!80!white]| \\
			|[fill=black!60!white]| \\
		...
		\node[left, at=(l1-1-1.south west)] {$10^5$};
		\node[left, at=(l1-2-1.south west)] {$10^4$};
		\node[left, at=(l1-3-1.south west)] {$10^3$};
		\node[left, at=(l1-4-1.south west)] {$10^2$};
		\node[left, at=(l1-5-1.south west)] {$10^1$};
		\node[above, at=(l1-1-1.north)] {probability};
	\end{tikzpicture}
\end{minipage}
%\hfill
