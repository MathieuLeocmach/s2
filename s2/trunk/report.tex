...
At polydispersities over $6-7\%$, the system need to fractionate to crystallise~\citep{Fasolo2003}. What is the bond order of the reference crystal is then unclear and may challenge our scenario. This is a situation reminiscent of binary hard sphere systems of size ratio close to one~\cite{Hopkins2011b,Hopkins2012} where growing bond order have not been reported~\cite{Charbonneau}. Although it is known that in binary systems locally favoured structures play a role in the slowing down of the dynamics~\cite{Coslovich2011,Malins2012}.
...
In principle, $s_2$ can be calculated separately for each particle $i$ in the system. In practice, this requires time averages to compute the pair correlation function of each particle, $g_i(r)$. In \cite{tanaka} we averaged on times comparable or longer than the $\alpha$ relaxation, leading to a quantity that was trivially a reflect of the dynamical heterogeneity. 
...
We confirmed that in the glass transition of polydisperse hard spheres the relevant static length is the correlation length of the crystalline bond order. Icosahedral order has a major role in the frustration against crystallisation, a role that increases with polydispersity. Neither of these physical aspects of the system could be described by two-body quantities like the two-body part of the free energy or of the structural entropy.
