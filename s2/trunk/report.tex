...
We study systematically the correlation between any possible two-body and multi-body order parameters on one hand, and the displacement of the particles on the other hand. For any scalar order parameter $x$, we define the mobility 
\begin{equation}
...
Figure~\ref{fig:distrib} shows the mobilities function of $f2$, $s_2$, $q_6$, $w_6$, $Q_6$ and $C$ for $\beta p\sigma^3=25$ and for a time difference corresponding to the $\alpha$-relaxation. The distribution of each order parameter is also indicated.
