...
\begin{equation}
	...
	\label{eq:Qlm}
\end{equation}
to kill locally incoherent orientations (icosahedral order)~\cite{mathieu_icosahedra} and use the resulting invariant $Q_6$ as an indication of crystallinity, more precisely, the degree of crystal-like rotational symmetry.
...
\begin{figure}
	...
	\caption{Probability distribution (red dash) and mobility (blue continuous) function of various order parameter at $\beta p\sigma^3=25$ and for a time difference corresponding to the $\alpha$-relaxation. Mobility is in unit of mean-square displacement. The limit of the shaded area is at one standard deviation from the mean value of the order parameter in the direction of ordering, $x^\ast$ (see below for its definition).}
	\label{fig:distrib}
\end{figure}
...
\begin{figure}
 ...
 \caption{Correlation between two-body and many-body parameters. Probability distribution functions in the $f_2$-$Q_6$ map (top) and in the $f_2$-$w_6$ map (bottom) for a metastable fluid with polydispersity $\delta=7\%$ and pressure $\beta p\sigma^3=23$. The parameters are not correlated, even in extreme values.}
...
\begin{figure}
	...
	\caption{Structure factors collapse on the Ornstein-Zernike form for (top) $f_2$ and (bottom) $Q_6$. Note how the former are almost similar across pressures. 
	For the latter, we can see the growth of the correlation length from the change in the relative position from the peak coming from the two-body correlation. }
...
\begin{figure}
	...
	\caption{Correlation length extracted for two-body and many-body scalar order parameters, function of the pressure. Only many-body correlation lengths are increasing (only slightly for $\xi_{w_6}$), while the lenghscale associated with the two-body quantities is almost constant.}
...
\begin{figure}
	...
	\caption{Visualisation of the ordered regions defined by the various order parameters. All pictures correspond to a thin slice ($5\sigma$) of the same configuration at $\beta p\sigma^3=23$ and $\delta=7\%$. Only $Q_6$ and $C$ show meaningful fluctuations. We can also see anti-correlation between ($Q_6$,$C$) and ($q_6$,$w_6$). }
	\label{fig:3D}
\end{figure}
...
their lenghscale does not grow appreciably with increasing pressure. Icosahedral assemblies of particles
...
controlling crystallization. This conjecture is further supported by our second finding on the mechanisms by which 
