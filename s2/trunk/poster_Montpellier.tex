...
\begin{minipage}[t][21.5\TPVertModule]{11\TPHorizModule}
	...
	\begin{tikzpicture}
		\begin{scope}[every node/.style={rectangle split},every text node part/.style={font=\normalsize\sffamily\scshape,Accent1}]
			\node (dyn) {Dynamics\nodepart{two}heterogeneities\nodepart{three}arrest};
			\node[anchor=base east] (twobody) at ($(dyn.base west) + (\textwidth,0)$) {Two-body\nodepart{two}$g(r), S(q)$};
			\node[anchor=base] (struct) at ($(dyn.base east)!0.5!(twobody.base west)$){Many-body\nodepart{two} crystal-like\nodepart{three}icosahedral};
		\end{scope}
		
		\draw[->, dashed, thick] (twobody) -- (struct.east|-twobody) node[midway, above] {?};
		\draw[->, thick] (struct) -- (dyn.east|-struct)  node[midway, above, font=\footnotesize\itshape] {Tanaka 2010} node[midway, below, font=\footnotesize\itshape] {Leocmach 2012};
	\end{tikzpicture}
%	
