...
The existence of a static (structural) length that would grow and accompany the dynamic heterogeneities is still not clear in the general case. However, in a class of system that includes polydisperse hard spheres, we have shown~\cite{tanaka} that some medium range bond orientational order reminiscent of the crystal exists in the supercooled liquid and grows toward the glass transition in the same way as the dynamical heterogeneity. The presence in glassy materials of structures locally reminiscent of crystals has been confirmed recently in amorphous silicon~\cite{Treacy2012}, a metallic glass~\cite{Hwang2012}, and an oxide glass, B$_2$O$_3$ \cite{ferlat2012hidden}. While bond orientational order is a member of the class of many-body
...
In Sec. \ref{sec:discussion}, we discuss implications of our results. In Sec. \ref{sec:summary} we summarize our work. 
...
\begin{equation}
f_2=\sum_i f_2(i)=-k_{\rm B}T\sum_i \log(v(i)/\lambda),
\end{equation}
where $k_{\rm B}$ is the Boltzmann constant and $\lambda$ is the thermal wavelength. To compute the free volume we follow previous
...
evident that, while crystal-like bond orientational order is rapidly suppressed with increasing
...
as at this pressure it is not possible to crystallize simulations at $\Delta=4\%$ at this pressure, while the
...
We believe that it is of crucial importance to incorporate many body correlations into theories, for describing 
