...
\begin{abstract}
Most of the liquid-state theories including glass-transition theories are constructed on the basis of two-body density correlation. 
However, we recently showed the importance of many-body correlations, particularly, bond orientational correlations in glass transition 
and crystallization. 
...
Together with our previous finding between structure-dynamics correlation, this indicates 
many-body correlations are key to the understanding of slow dynamics associated with glass transition. 
We also reveal that the stability against crystallization with increasing polydispersity
is due to an increasing population of icosahedral arrangements of particles, competing with regions of high crystal-like bond orientational order 
which instead promote crystallization. 
Our results suggest an intimate link between glass transition and crystallization at least in this type of systems and 
crucial roles of many-body correlations in these phenomena. 
...
The existence of a static (structural) length that would grow and accompany the dynamic heterogeneities is still not clear in the general case. However, in a class of system that includes polydisperse hard spheres, we have shown~\cite{tanaka} that some medium range bond orientational order reminiscent of the crystal exists in the supercooled liquid and grows toward the glass transition in the same way as the dynamical heterogeneity. 
This suggests a deep link between vitrification and crystallization \cite{TanakaGJPCM}. 
The presence in glassy materials of structures locally reminiscent of crystals has also been confirmed recently in amorphous silicon~\cite{Treacy2012} and in a metallic glass~\cite{Hwang2012}.
...
In principle, $s_2$ can be calculated separately for each particle $i$ in the system. In practice, this requires time averages to compute the pair correlation function of each particle, $g_i(r)$. In \cite{tanaka,watanabe_walls} we averaged on times comparable or longer than the $\alpha$ relaxation, leading to a quantity that was trivially a reflect of the dynamical heterogeneity. 
...
We study systematically two-body ($s_2$, $f_2$) and many-body ($q_6$, $w_6$, $Q_6$, $C$) scalar order parameter fields for our highest pressure ($\beta p\sigma^3=25$). Figure~\ref{fig:distrib} shows the probability distribution of these parameters, always skewed toward the ordered side (shaded in Fig.~\ref{fig:distrib}).
...
Both fast and sow particles are a priori important to describe dynamic heterogeneities, thus we cannot discard the relevance of either two-body or many-body parameters based on this analysis.
...
place, with polydispersity favoring the latter. The nature of the competition can be seen
...
crystalline curve. The behavior of the fluid curve is due to an increased population of
...
We confirmed that in the glass transition of polydisperse hard spheres the relevant static length is the correlation length of the crystal-like bond orientational order. 
